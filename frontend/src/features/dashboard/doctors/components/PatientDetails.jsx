...
const PatientDetails = () => {
  ...
  const handleStartConsultation = async () => {
    ...
    try {
      ...
      dispatch(
        setSuccessFeedback({
          message: res.message || "Start doctor consultation",
        })
      );
    ...
    <div className="bg-white rounded-2xl p-6 w-full max-w-lg">
      {isSuccess ? (
        <div className="text-center">
          <SuccessFeedback />
          <h1 className="text-2xl">{isSuccess && "Updated!"}</h1>
          <p className="text-textSecondary text-center">
            {isSuccess && (message || "Start doctor consultation")}
          </p>
        </div>
      ) : (
        <>
